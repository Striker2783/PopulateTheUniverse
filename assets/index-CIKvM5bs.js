...
class Totaller extends Observer {
  constructor(v) {
    super(v);
    __publicField(this, "_total", new Observer(Decimal.dZero));
...
class Rater extends Totaller {
  constructor() {
    super(...arguments);
    __publicField(this, "_max_changes", 5);
    __publicField(this, "values", []);
    __publicField(this, "_rate", new Observer(Decimal.dZero));
  }
  tick() {
    if (this.values.length >= this._max_changes)
      this.values.shift();
  ...
  update_rate() {
    ...
    const sum_rates = this.values.slice(0, -1).map((curr, index) => {
      const next = this.values[index + 1];
      const diff_time = (next.t - curr.t) / 1e3;
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        unref(page) == 0 ? (openBlock(), createBlock(_sfc_main$2, { key: 0 })) : unref(page) == 1 ? (openBlock(), createBlock(_sfc_main$1, { key: 1 })) : createCommentVNode("", true),
